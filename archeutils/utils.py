...
TOP_COL_RDF = getattr(settings, "TOP_COL_RDF", None)
repo_schema = (
...
def as_arche_graph(res):
    ...
    g.add((sub, acdh_ns.hasTitle, Literal(f"{res}", lang=ARCHE_LANG)))
    if "[internal-id]" not in res.macr_nr:
        signatur = f"{ARCHIV_NAME}, Missing Air Crew Reports No.: {res.macr_nr}"
    else:
        signatur = f"{ARCHIV_NAME}"
    g.add((sub, acdh_ns.hasNonLinkedIdentifier, Literal(f"{signatur}")))
    g.add(
        (
            sub,
            acdh_ns.hasNonLinkedIdentifier,
            Literal(f"Legacy Database ID: bomber/{res.id}"),
        )
    )
    if res.name:
        g.add(
            (sub, acdh_ns.hasAlternativeTitle, Literal(f"{res.name}", lang=ARCHE_LANG))
        )
    if res.comment:
        g.add((sub, acdh_ns.hasNote, Literal(f"{res.comment}", lang="de")))
    g.add(
        (
            sub,
            acdh_ns.hasCategory,
    ...
    for x in res.get_squad_group_airforce:
        ...
        g.add((sub, acdh_ns.hasActor, cur_uri))
        cur_g.add((cur_uri, RDF.type, acdh_ns.Organisation))
        cur_g.add((cur_uri, acdh_ns.hasTitle, Literal(f"{x.written_name}", lang="en")))
        cur_g.add(
            (
                cur_uri,
                acdh_ns.hasDescription,
                Literal(
                    f"'{x.written_name}' is a military unit mentioned in the DAACDA Project",
                    lang="en",
                ),
            )
        )
    ...
    for x in res.get_prisons.all():
        ...
        g.add((sub, acdh_ns.hasActor, cur_uri))
        cur_g.add((cur_uri, RDF.type, acdh_ns.Organisation))
        cur_g.add((cur_uri, acdh_ns.hasTitle, Literal(f"{x.name}", lang="de")))
        g = g + cur_g

    for x in res.has_crew.all():
        crew_g = Graph()
        crew_uri = URIRef(f"{ARCHE_BASE_URL}/persons/{x.id}")
        g.add((sub, acdh_ns.hasActor, crew_uri))
        crew_g.add((crew_uri, acdh_ns.hasTitle, Literal(f"{x}", lang="und")))
        crew_g.add((crew_uri, acdh_ns.hasLastName, Literal(f"{x.name}", lang="und")))
        crew_g.add(
            (
                crew_uri,
    ...
    for x in res.get_places.all():
        try:
            ...
            pl_g.add((pl_uri, acdh_ns.hasTitle, Literal(f"{pl}", lang="de")))
            pl_g.add((pl_uri, RDF.type, acdh_ns.Place))
            g = g + pl_g
            g.add((sub, acdh_ns.hasSpatialCoverage, pl_uri))
        except:
            print("OJE")
            pass
    ...
    col.add((col_sub, acdh_ns.hasTitle, Literal(f"{res.squadron}", lang=ARCHE_LANG)))
    col.add(
        (
            col_sub,
            acdh_ns.hasDescription,
