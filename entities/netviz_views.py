...
def inst_planes_json(request, pk):
    ...
    if (
        current_object.children_institutions.all().count() > 0
        and current_object.parent_institution
    ):
        bombers = Bomber.objects.filter(squadron__parent_institution=current_object)
...
def airstrike_time_json(request):
    my_data = collections.Counter(
        [x.crash_date_data() for x in Airstrike.objects.all()]
    )
